...
(defn notify-msg
  ...
  (let [bill (state :bill)
        buffer (state :buffer)
        amount-needed (get bill ware 0)
        current-buffer-amount (get buffer ware 0)]
    ;; Проверяем, нужен ли этот товар для производства
    (if (and (> amount-needed 0) (< current-buffer-amount amount-needed))
      ;; Пытаемся забрать недостающее количество из хранилища
      (let [needed-amount (- amount-needed current-buffer-amount)
            amount-to-take (min needed-amount amount)]
        (try
          ;; Пытаемся атомарно забрать товары из атома хранилища
          (let [old-storage @storage-atom
                new-storage (swap! storage-atom 
                                 (fn [current-storage]
                                   (let [available (min current-storage amount-to-take)
                                         result (- current-storage available)]
                                     (if (< result 0)
                                       (throw (IllegalStateException. "Storage cannot be negative"))
                                       result))))
                actually-taken (- old-storage new-storage)
                new-buffer (update buffer ware + actually-taken)]
            ;; Проверяем, достаточно ли товаров для производства
            (let [can-produce (every? (fn [[ware-name needed]]
                                        (>= (get new-buffer ware-name 0) needed))
                                      bill)]
              (if can-produce
                ;; Запускаем производственный цикл
                (let [target-storage (state :target-storage)
                      production-amount (state :amount)
                      production-duration (state :duration)
                      factory-agent *agent*]
                  ;; Имитируем производственный цикл в отдельном потоке
                  (future
                    (Thread/sleep production-duration)
                    ;; После завершения цикла удаляем использованные товары из буфера
                    ;; и уведомляем целевое хранилище
                    (send factory-agent
                          (fn [current-state]
                            (let [current-bill (current-state :bill)
                                  current-buffer (current-state :buffer)
                                  updated-buffer (reduce-kv 
                                                   (fn [acc k needed]
                                                     (assoc acc k (- (get acc k 0) needed)))
                                                   current-buffer
                                                   current-bill)]
                              ;; Уведомляем целевое хранилище о произведенных товарах
                              (send ((current-state :target-storage) :worker) supply-msg (current-state :amount))
                              ;; Возвращаем обновленное состояние с очищенным буфером
                              (assoc current-state :buffer updated-buffer)))))
                  ;; Возвращаем состояние с обновленным буфером
                  (assoc state :buffer new-buffer))
                ;; Недостаточно товаров, просто обновляем буфер
                (assoc state :buffer new-buffer))))
          ;; Обрабатываем исключение валидации
          (catch IllegalStateException e
            ;; Не удалось забрать товары, возвращаем состояние без изменений
            state)
          (catch Exception e
            ;; Другие исключения тоже обрабатываем
            state)))
      ;; Этот товар не нужен для производства, возвращаем состояние без изменений
      state)))
